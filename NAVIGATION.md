...
- **Frontend:** `studio.js` + `studio.css` for method picker, common-video matrix, preparation status, packages and organizer analysis; only published/closed/archived Campaigns expose a share link. `blind.html` + `blind.js` + `blind.css` is isolated from the main navigation; preparing links show an auto-refreshing wait state, while draft/failed links never request reviews.
