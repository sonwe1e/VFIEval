...
发布是可恢复的 preparation job：先在 staging 中逐视频深度校验，再生成 `.vfieval/evaluations/{campaign_id}` 与 SHA-256 manifest，最后把冻结文件登记为 `source_kind="evaluation_package"` 并原子发布任务。新包只冻结 GT、Pred A、Pred B 三路；旧包中的 Diff 文件和 manifest 字段继续可读。indexed VFI 的 GT 按映射重编码；Pred A/B 只有在完整帧序列、H.264/yuv420p、零旋转、CFR、尺寸、FPS 和相对时间轴都严格一致时才成对 remux，任一路不合格或失败都会让两路一起回退编码。解码缓存使用可信内容摘要、大小和 mtime 身份，最终发布前仍重新校验全部源内容；阶段进度会分别展示 alignment、身份校验、hash、decode-cache、remux/encode、输出校验和源稳定性耗时。同一服务进程一次只冻结一个 Campaign，Item 内仍使用有界并行。
...
视频任务由任务范围内的 sticky 控制条统一控制三路媒体的播放、暂停、seek、播放速率和循环，以 GT 作为同步时钟；支持 `requestVideoFrameCallback` 的浏览器会按展示帧检查漂移，其他浏览器使用 `timeupdate` 回退。任一路缓冲或意外暂停时页面会对齐暂停三路，全部就绪后自动恢复；浏览器策略拒绝自动播放时会提示点击继续，只有真实解码或网络错误才显示播放失败。需要最严格观察时应暂停到目标帧，帧序列任务则始终让三张图共享同一帧索引。
