...
部署或排错时先运行完整环境检查；遇到 Run/Campaign 异常时可生成已脱敏的支持包：
...
运行日志按 JSONL 轮转写入 `.vfieval/logs/`。页面内部错误会显示 `support_id` 与
`request_id`，可直接用它们定位服务端日志；诊断包会脱敏工作区路径与令牌字段。
...
- 推理表单使用保守起点：检测到 NPU 时默认选择可用多卡，精度为 `auto`，总 batch 与每设备 batch 均为 `1`；用户可以在看过深度预检后再提高吞吐。
...
- 全参考视频评测使用 `midpoint-triplet-v2`：`frame_step=s` 时只生成真实对称的 `(i, i+s, i+2s)` 三元组，因此 N 帧产生 `N-2s` 个可信样本；末尾不再用钳制帧伪造 GT。缺少该契约或含历史 `clamped_boundary` 样本的 Run 仍可查看，但必须用当前版本重跑后才能创建或发布新的盲测 Campaign。
- 预检会显示设备与卡数、精度、单设备 batch、解码后端、评测契约、分辨率、显存/主机内存、磁盘余量和产物空间预算。数字是资源规划下界，不包含模型参数、激活和分配器工作区；高风险配置必须在看过具体数值后确认，提交时携带匹配的 `risk_ack_fingerprint`。
- Run、上传和盲测发布会统一预留磁盘安全余量；盲测预算按 GT/A/B 的帧数与分辨率估算 PNG 解码展开、冻结视频和 staging，而不只看低码率源文件大小。容量不足会在写盘前返回 `507 StorageCapacityError`，不会留下半发布任务。
...
- 两个特征距离适配器都要求权重名称与形状完整匹配，并在加载后执行确定性一致性检查；详细的权重、实现指纹和缓存契约见 [LPIPS feature-distance adapters](docs/feature-distance-metrics.md)。
...
发布请求会先持久化 `preparing/queued` 再向浏览器回包；浏览器或反向代理在回包期间断开所产生的 `Broken pipe` 只按客户端断线处理，不会把已排队的 preparation 标成失败。Studio 仅为 `published`、`closed` 或 `archived` Campaign 显示参与链接；直接打开 `preparing` 链接会显示等待状态并自动刷新，`draft` 或 `failed` 链接不会误进入完成页或请求 reviews。若冻结编码器本身提前关闭 stdin，失败详情和服务终端会记录 Campaign、输出 sink、FFmpeg 退出码及 stderr，而不是只保留原始 `[Errno 32]`。
...
使用 Windows 管理机向局域网转发完整服务时，按 [Windows TCP relay 部署与自检](docs/deployment-windows-relay.md) 配置；脚本会同时验证主页、健康接口、盲评静态资源和 HTTP Range。
