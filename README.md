...
发布是可恢复的 preparation job：先在 staging 中逐视频深度校验，再生成 `.vfieval/evaluations/{campaign_id}` 与 SHA-256 manifest，最后把冻结文件登记为 `source_kind="evaluation_package"` 并原子发布任务。视频冻结会在编码格式、像素格式、旋转、尺寸、CFR 和 identity 时间映射全部满足严格条件时使用私有 FFmpeg remux 快路；GT 独立判断，Pred A/B 共同命中才走快路。其他情况或 remux 兼容性失败会回退到逐帧流式物化：每帧只读取 GT/A/B 一次，在内存完成尺寸校验、LANCZOS 和 Diff，再通过有界队列编码五路成品，视频主路径不创建临时 PNG；帧序列模式则直接写一次最终 PNG。该设计减少重复解码、PNG I/O 和哈希，但实际耗时仍取决于素材格式、分辨率、帧数、是否缩放以及 CPU 和磁盘性能。
...
参与者只打开独立的 `/evaluate/{opaque_token}` 页面，不加载主导航、Media 或管理分析。浏览器生成稳定 evaluator UUID，评测员填写显示名后领取带可续期 lease 的任务；任务、assignment 和媒体 URL 都使用 opaque token，投票前不暴露模型、checkpoint、Run、方法标签或真实 asset/task id。左右顺序按任务与评测员稳定随机。投票可选 A、B 或平局，并记录质量原因、置信度和备注。
...
视频任务由一组统一控件控制三路媒体的播放、暂停、seek、播放速率和循环，以 GT 作为同步时钟；支持 `requestVideoFrameCallback` 的浏览器会按展示帧检查漂移，其他浏览器使用 `timeupdate` 回退。独立浏览器解码器无法承诺样本级绝对同步，缓冲时页面会暂停三路媒体以保护对齐；需要最严格观察时应暂停到目标帧，帧序列任务则始终让三张图共享同一帧索引。
